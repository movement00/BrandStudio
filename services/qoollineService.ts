...
// ═══ COLOR PALETTE AGENT — Extract + harmonize colors ═══
export const analyzeColorPalette = async (
  blueprintLayers: any[],
  brand: Brand,
): Promise<string> => {
  const ai = getAI();
  const colorLayers = blueprintLayers.filter((l: any) => l.style?.color || l.style?.backgroundColor || l.type === 'background');
  const response = await ai.models.generateContent({
    model: 'gemini-3-pro-preview',
    contents: { parts: [{ text: `Sen renk teorisi uzmanisin. Referans gorseldeki renkleri marka paleti ile harmonize et.

REFERANS GORSEL RENKLERI:
${colorLayers.map(l => `- [${l.type}] ${l.style?.color || ''} ${l.style?.backgroundColor || ''} ${l.style?.gradient || ''}`).join('\n')}

MARKA PALETI:
${brand.palette.map(c => `- ${c.name}: ${c.hex}`).join('\n')}

KARAR VER (2-3 cumle):
- Arka plan hangi renk olmali? (referanstan mi marka paletinden mi?)
- Metin renkleri ne olmali? (kontrast icin)
- Vurgu rengi hangisi?
- Referans renkleri ile marka renkleri nasil dengelenmeli?
...
// ═══ COMPOSITION AGENT — Analyze layout + golden ratio ═══
export const analyzeComposition = async (
  ...
  const response = await ai.models.generateContent({
    model: 'gemini-3-pro-preview',
    contents: { parts: [{ text: `Sen profesyonel bir kompozisyon direktörusun. Gorsel icin ideal metin yerlestirmesini belirle.

BLUEPRINT:
- Layout: ${blueprint.layout?.type || 'bilinmiyor'}
- Kompozisyon: ${blueprint.compositionNotes || 'bilinmiyor'}
- Katman sayisi: ${blueprintLayers.length}

KATMANLAR:
${blueprintLayers.map(l => `- [${l.type}] "${(l.content || '').slice(0, 30)}" pozisyon: ${l.position?.anchor || l.position?.y || 'bilinmiyor'} boyut: ${l.size?.width || ''}`).join('\n')}

KOMPOZISYON KURALLARI:
- Ucte bir kurali (rule of thirds)
- Altin oran
- Goz akis yonu (Z-pattern veya F-pattern)
- Bos alan (whitespace) dengesi

KARAR VER (2-3 cumle):
- Metinler nereye yerlesecek? (ust/alt/sol/sag)
- Ana odak noktasi nerede?
- Bos alanlar nerede — oraya metin koyulmali
- Objelerin uzerine metin KONMAMALI
...
// ═══ MOOD BOARD AGENT — Generate mood/direction for campaign ═══
export const generateMoodBoard = async (
  campaign: QoollineCampaign,
  brand: Brand,
): Promise<{ mood: string; direction: string; keywords: string[] }> => {
  const ai = getAI();
  const response = await ai.models.generateContent({
    model: 'gemini-3-pro-preview',
    contents: { parts: [{ text: `Sen kreatif direktorusun. Bu kampanya icin mood board yonlendirmesi olustur.

MARKA: ${brand.name} — ${brand.industry}
TON: ${brand.tone}
KAMPANYA: ${campaign.type} — "${campaign.core}"

Mood board olustur:
1. MOOD: Genel hava/duygu (2-3 kelime)
2. DIRECTION: Gorsel yonlendirme (1-2 cumle — nasil bir gorsel olmali)
3. KEYWORDS: Gorsel arama anahtar kelimeleri (5 kelime, virgullu)

JSON olarak don:
{"mood":"...","direction":"...","keywords":["k1","k2","k3","k4","k5"]}

SADECE JSON don.` }] },
    config: { responseMimeType: 'application/json' }
  });
  try { return JSON.parse(response.text || '{}'); } catch { return { mood: '', direction: '', keywords: [] }; }
};

// ═══ A/B TEST AGENT — Generate variant approaches ═══
export const generateABVariants = async (
  campaign: QoollineCampaign,
  brand: Brand,
): Promise<{ approach: string; editPromptAddition: string }[]> => {
  const ai = getAI();
  const response = await ai.models.generateContent({
    model: 'gemini-3-pro-preview',
    contents: { parts: [{ text: `Sen A/B test uzmanisin. Bu kampanya icin 3 FARKLI gorsel yaklasimi olustur.

MARKA: ${brand.name}
KAMPANYA: ${campaign.type} — "${campaign.core}"
RENK PALETI: ${brand.palette.map(c => `${c.hex}`).join(', ')}

Her yaklasim FARKLI olmali:
- Varyant A: Farkli renk agirlik (orn: mor baskin)
- Varyant B: Farkli tipografi (orn: serif vs sans)
- Varyant C: Farkli yerlesim (orn: metin solda vs ortada)

JSON array olarak don:
[{"approach":"kisa aciklama","editPromptAddition":"modele eklenecek 1 cumle talimat"}]
